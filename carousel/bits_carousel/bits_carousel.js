...
export default can.Component.extend({
	...
	scope : {
		...
		carouselContentWidth : function(){
			var width = this.attr('bits').attr('length') * CARD_WIDTH;
		...
		carouselNext : function(ctx, el){
			...
			var next = this.attr('fromLeft') + CARD_WIDTH;
			if(next > this.maxFromLeft()){
				this.loadNextPage();
				next = next + CARD_WIDTH;
			}
		...
		carouselPrev : function(){
			this.attr('fromLeft', Math.max(0, this.attr('fromLeft') - CARD_WIDTH));
		}
	},
	helpers : {
